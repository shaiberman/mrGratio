function f = nc_PlotModelFitsJ_fig2(coefs,valName,fgNames,fgnums,color,R)
...
for ii = fgnums
    ...
    if ismember(pnum,1:3:24)
        ylabel(YLab,'fontname','times','fontsize',17);
    ...
    if strcmp(valName,'gr') || strcmp(valName,'gratio')
        in=~isnan(coefs(ii).x) & ~isnan(coefs(ii).y);
        [R,P] =corrcoef(coefs(ii).x(in), coefs(ii).y(in));
        tit=['\rho = ',num2str(round(R(2),2)), ' ,  p = ',num2str(round(P(2),4))];
        if strcmp(mat,'max')
    ...
    % title(['R(CV)=',num2str(R(ii))],'fontsize',8,'fontname','times')
